        ...
        if i == ID:
            ...
            while True:
                ...
                if angulo == 90:
                    ...
                    imagen_norm = aplicacion / np.max(aplicacion)
                    valor_im = imagen_norm * 255
                    aplicacion = valor_im.astype(np.uint8)
                    ruta_guardado = f"ImagenesModificadas/imagen_rotada_{angulo}.jpg"
                    cv2.imwrite(ruta_guardado, aplicacion) #Guardar la imagen rotada
    ...
    def transformacion(self,clave):
        for i in dicomIm:
            if i == clave:
                ...
                xt=cv2.putText(tmorfolog,f"IMAGEN BINARIZADA - KERNEL > {x,y} - UMBRAL {Umb}",(10,30),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2) #Titulo de la imagen
                ruta_guardado = f"ImagenesModificadas/imagen_binarizada.jpg"
                cv2.imwrite(ruta_guardado, xt) #Guardar la imagen transformada
                plt.figure(figsize=(15,8),facecolor="lightblue")
                plt.subplot(2,2,1)
